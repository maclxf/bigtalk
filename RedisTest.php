...
$redis->lpop    PHP版本从左侧删除一个元素      成功返回 被删除元素的值*/
// 往列表中添加一个值
...
$redis->sadd    PHP版本将元素添加到集合               成功返回1，失败（重复）返回0
...
hgetall 获取散列所有的键值对
hdel    删除给定键
$redis->hset    PHP版本，添加元素到散列           设置成功返回1，若重复的返回0
$redis->hget    PHP版本，获取指定键的值
$redis->hgetall PHP版本，获取散列所有的键值对
$redis->hdel    PHP版本，删除给定键               删除成功返回1，否则返回0
*/
//我们先插入一个键值对到hash-key散列。1代表插入成功；0元素代表已经存在
var_dump($redis->hset('hash-key','sub-key1','value1'));// 设置成功返回1，若重复的返回0
var_dump($redis->hset('hash-key','sub-key2','value2'));// 设置成功返回1，若重复的返回0
...
var_dump($redis->hdel('hash-key','sub-key1')); // 该键不存在，返回int 0
